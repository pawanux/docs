...
Go to [https://tryunleashx.com](https://tryunleashx.com) and open the agent you want to give OCR to.
</Step>
...
![01 ocr tools](/images/mcp/ocr/01-tools.png)

| Tool | What it does | Changes data? |
|---|---|---|
| `ocr_extract_text` | Extract plain text from an image given by path, URL, or base64 | No |
| `ocr_from_url` | Download an image from a URL and extract its text | No |
| `ocr_from_file` | Extract text from a local image file | No |
| `ocr_extract_with_boxes` | Extract text plus bounding boxes and confidence details | No |
| `ocr_batch_extract` | Extract text from every image in a directory | No |
| `ocr_supported_languages` | List the languages OCR can read | No |
| `ocr_get_info` | Report OCR server status and configuration | No |
| `ocr_preprocess_image` | Clean up an image (resize, grayscale, threshold, blur) before extraction | No |

<Note>
Every OCR tool is read-only — it reads pixels and returns text. `ocr_preprocess_image` writes a temporary working file but does not modify your original image or any stored data.
</Note>

## 6. Example usage

**"Read this business card and pull out the name, company, and phone number."**
→ Runs `ocr_from_url` (or `ocr_extract_text`) to extract the text, then the agent parses the fields.

**"Go through every receipt image in this folder and give me the totals."**
→ Runs `ocr_batch_extract` across the directory and the agent reads each total from the returned text.
...
| Empty or partial text | Image is low-resolution, blurry, or rotated | Run `ocr_preprocess_image` first, or supply a clearer image |
...
Many, automatically. Call `ocr_supported_languages` for the current list (English, Spanish, French, German, Chinese, Japanese, Arabic, Hindi, and more).
