...
Open **Workflows → New Workflow**. On the canvas, click **+ Add Trigger**. In the **Paths** panel, open the **Apps** tab and select **Zoho Desk** — its **Triggers** are listed underneath. Use the search box if you have many connectors.

![Zoho Desk triggers in the Apps panel](/images/mcp/zoho-desk/workflows/06-apps-triggers.png)
</Step>
<Step title="Add an action node">
Click the **+** below any node to add a step, then pick **Zoho Desk** again — this time the panel lists its **Actions**.

![Zoho Desk actions in the Apps panel](/images/mcp/zoho-desk/workflows/07-apps-actions.png)
</Step>
