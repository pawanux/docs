...
Go to [https://tryunleashx.com](https://tryunleashx.com) and open the agent that should generate avatar videos.
</Step>
...
|---|---|---|
| `heygen_text_to_video` | Generate a talking-avatar video from a text script | ✏️ Yes |
| `heygen_image_to_video` | Generate a talking-photo video from an image plus text | ✏️ Yes |
| `heygen_get_info` | Report HeyGen configuration and readiness | No |
...
→ Runs `heygen_text_to_video` with the script (and optional avatar/voice), returning a video ID or URL.

**"Turn this headshot into a short talking-photo intro."**
→ Runs `heygen_image_to_video` with the image URL and the script.
