...
|---|---|---|
| `wa_send_message` | Send a plain WhatsApp text message | ✏️ Yes |
| `wa_list_templates` | List your approved message templates | No |
| `wa_send_template` | Send an approved template message with optional variables | ✏️ Yes |
| `wa_send_image_url` | Send an image by public URL (optional caption) | ✏️ Yes |
| `wa_send_document` | Send a document by uploaded media ID | ✏️ Yes |
| `wa_send_document_url` | Send a document by public URL | ✏️ Yes |
| `wa_send_audio` | Send an audio/voice message by media ID | ✏️ Yes |
| `wa_send_voice_message` | Download audio from a URL, upload it, and send as a voice note | ✏️ Yes |
| `wa_send_video` | Send a video by media ID (optional caption) | ✏️ Yes |
| `wa_mark_read` | Mark an inbound message as read | ✏️ Yes |
| `get_insurance_detail` | Get car insurance quotes for a vehicle registration number | No |
| `zimyo_employee_detail` | Look up Zimyo employee details by phone number | No |

<Note>
`get_insurance_detail` and `zimyo_employee_detail` are helper lookups bundled into this server for specific business workflows (insurance quoting and HR employee lookup). They don't send WhatsApp messages themselves.
</Note>

## 6. Example usage

**"Text +91 98765 43210 their order #4521 has shipped."**
Behind the scenes: `wa_send_message` (UnleashX normalizes the number and posts to the Cloud API).

**"Send the 'appointment_reminder' template to this customer for 3 PM tomorrow."**
Behind the scenes: `wa_list_templates` (confirm the template) → `wa_send_template` (with the time as a variable).
...
| Template not found / not approved | The template name/language is wrong or not yet approved | Use `wa_list_templates` to confirm the exact name and language; approve it in Meta if pending |
