...
Open **Workflows → New Workflow**. On the canvas, click **+ Add Trigger**. In the **Paths** panel, open the **Apps** tab and select **Hyperbrowser** — its **Triggers** are listed underneath. Use the search box if you have many connectors.

![Hyperbrowser triggers in the Apps panel](/images/mcp/hyperbrowser/workflows/06-apps-triggers.png)
</Step>
<Step title="Add an action node">
Click the **+** below any node to add a step, then pick **Hyperbrowser** again — this time the panel lists its **Actions**.

![Hyperbrowser actions in the Apps panel](/images/mcp/hyperbrowser/workflows/07-apps-actions.png)
</Step>
