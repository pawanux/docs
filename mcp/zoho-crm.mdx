...
|---|---|---|
| `zoho_crm_get_lead` | Retrieves a single lead by ID. | No |
| `zoho_crm_get_leads` | Retrieves multiple leads. | No |
| `zoho_crm_create_lead` | Creates a new lead. | ✏️ Yes |
| `zoho_crm_update_lead` | Updates an existing lead. | ✏️ Yes |
| `zoho_crm_convert_lead` | Converts a lead into a contact/account/deal. | ✏️ Yes |
| `zoho_crm_get_contact` | Retrieves a single contact by ID. | No |
| `zoho_crm_get_contacts` | Retrieves multiple contacts. | No |
| `zoho_crm_create_contact` | Creates a new contact. | ✏️ Yes |
| `zoho_crm_update_contact` | Updates an existing contact. | ✏️ Yes |
| `zoho_crm_get_account` | Retrieves a single account by ID. | No |
| `zoho_crm_get_accounts` | Retrieves multiple accounts. | No |
| `zoho_crm_create_account` | Creates a new account. | ✏️ Yes |
| `zoho_crm_update_account` | Updates an existing account. | ✏️ Yes |
| `zoho_crm_get_deal` | Retrieves a single deal by ID. | No |
| `zoho_crm_get_deals` | Retrieves multiple deals. | No |
| `zoho_crm_create_deal` | Creates a new deal. | ✏️ Yes |
| `zoho_crm_update_deal` | Updates an existing deal. | ✏️ Yes |
| `zoho_crm_search_records` | Searches records within a module by criteria. | No |
| `zoho_crm_search_global` | Performs a global search across all or specific modules. | No |
| `zoho_crm_add_note` | Adds a note to a record. | ✏️ Yes |
| `zoho_crm_get_notes` | Retrieves notes on a record. | No |
| `zoho_crm_upload_attachment` | Uploads a file attachment to a record. | ✏️ Yes |
| `zoho_crm_update_note` | Updates an existing note. | ✏️ Yes |
| `zoho_crm_log_call` | Logs a call activity against a record. | ✏️ Yes |
| `zoho_crm_create_task` | Creates a task. | ✏️ Yes |
| `zoho_crm_get_tasks` | Retrieves tasks. | No |
| `zoho_crm_search_users` | Searches CRM users. | No |
| `zoho_crm_create_user` | Creates a new CRM user. | ✏️ Yes |
| `zoho_crm_delete_user_manual` | Deletes a specified CRM user. | ⚠️ Yes |
| `zoho_crm_delete_user_from_list` | Deletes users from a provided list. | ⚠️ Yes |
| `zoho_crm_create_object` | Creates a record in any (including custom) module. | ✏️ Yes |
| `zoho_crm_update_object` | Updates a record in any module. | ✏️ Yes |
| `zoho_crm_list_objects` | Lists records from any module. | No |
| `zoho_crm_delete_object_manual` | Deletes a specified record. | ⚠️ Yes |
| `zoho_crm_delete_objects_from_list` | Deletes records from a provided list. | ⚠️ Yes |
| `zoho_crm_list_related_records` | Lists records related to a given record. | No |
| `zoho_crm_get_modules` | Lists the CRM modules. | No |
| `zoho_crm_get_fields` | Lists fields for a module. | No |
| `zoho_crm_get_field` | Retrieves a single field's metadata. | No |
| `zoho_crm_get_users` | Lists CRM users. | No |
| `zoho_crm_list_leads` | Lists leads. | No |
| `zoho_crm_list_contacts` | Lists contacts. | No |
| `zoho_crm_list_accounts` | Lists accounts. | No |
| `zoho_crm_list_deals` | Lists deals. | No |
| `zoho_crm_list_tasks` | Lists tasks. | No |

## 6. Example usage

**"Create a lead for Jordan Lee at Acme Corp, then log that I just had an intro call with them."**
→ Runs `zoho_crm_create_lead` to add the lead, then `zoho_crm_log_call` to record the call activity.

**"Find the deal with Globex and move it to the Negotiation stage with a note about the new pricing."**
→ Runs `zoho_crm_search_records` (or `zoho_crm_search_global`) to find the deal, `zoho_crm_update_deal` to change the stage, and `zoho_crm_add_note` to attach the pricing note.
