...
Open **Workflows → New Workflow**. On the canvas, click **+ Add Trigger**. In the **Paths** panel, open the **Apps** tab and select **Brevo** — its **Triggers** are listed underneath. Use the search box if you have many connectors.
</Step>
<Step title="Add an action node">
Click the **+** below any node to add a step, then pick **Brevo** again — this time the panel lists its **Actions**.

![Brevo actions in the Apps panel](/images/mcp/brevo/workflows/07-apps-actions.png)
</Step>
