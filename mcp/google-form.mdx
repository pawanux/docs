...
|---|---|---|
| `gf_create_form` | Create a new form. | ✏️ Yes |
| `gf_get_response` | Get a single response by ID. | No |
| `gf_list_responses` | List submitted responses for a form. | No |
| `gf_get_form_properties` | Get a form's properties/settings. | No |
| `gf_delete_response` | Delete response row(s) from the linked sheet. | ⚠️ Yes |
| `gf_update_response` | Update a response row in the linked sheet. | ✏️ Yes |
| `gf_delete_form` | Delete a form. | ⚠️ Yes |
| `gf_list_columns` | List a form's columns. | No |
| `gf_list_forms` | List available forms. | No |
| `gf_list_sheets` | List sheets (tabs) in a linked spreadsheet. | No |
| `gf_list_spreadsheets` | List available spreadsheets. | No |
| `gf_add_text_question` | Add a text question to a form. | ✏️ Yes |
| `gf_add_multiple_choice_question` | Add a multiple-choice question to a form. | ✏️ Yes |

## 6. Example usage

> **"Create a feedback form titled 'Event Survey' with a text question 'What went well?' and a multiple-choice rating question."**
>
> Runs `gf_create_form`, then `gf_add_text_question` and `gf_add_multiple_choice_question`.

> **"Show me the latest responses to the Event Survey."**
>
> Runs `gf_list_forms` to find the form, then `gf_list_responses`.
