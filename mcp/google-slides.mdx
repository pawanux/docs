...
### Use Google Slides in a Workflow

Once connected, you can add **Google Slides** to any automation from the **Workflows** builder. Its triggers and tools appear in the Apps panel, marked with an **MCP** badge.

<Steps>
<Step title="Add a trigger node">
Open **Workflows → New Workflow**. On the canvas, click **+ Add Trigger**. In the **Paths** panel, open the **Apps** tab and select **Google Slides** — its **Triggers** are listed underneath. Use the search box if you have many connectors.
</Step>
<Step title="Add an action node">
Click the **+** below any node to add a step, then pick **Google Slides** again — this time the panel lists its **Actions**.

![Google Slides actions in the Apps panel](/images/mcp/google-slides/workflows/07-apps-actions.png)
</Step>
