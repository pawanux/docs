...
Open **Workflows → New Workflow**. On the canvas, click **+ Add Trigger**. In the **Paths** panel, open the **Apps** tab and select **Typeform** — its **Triggers** are listed underneath. Use the search box if you have many connectors.
</Step>
<Step title="Add an action node">
Click the **+** below any node to add a step, then pick **Typeform** again — this time the panel lists its **Actions**.

![Typeform actions in the Apps panel](/images/mcp/typeform/workflows/07-apps-actions.png)
</Step>
