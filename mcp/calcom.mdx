...
![01 calcom data connector](/images/mcp/calcom/01-data-connector.png)

## 5. Available tools

| Tool (technical) | What it does | Changes data? |
|---|---|---|
| `calcom_get_booking` | Gets details of a single booking | No |
| `calcom_list_bookings` | Lists bookings | No |
| `calcom_list_bookings_with_changes` | Lists bookings including recent changes | No |
| `calcom_create_booking` | Creates a new booking | ✏️ Yes |
| `calcom_reschedule_booking` | Reschedules an existing booking | ✏️ Yes |
| `calcom_delete_booking` | Cancels/deletes a booking | ⚠️ Yes |
| `calcom_list_event_types` | Lists your event types | No |
| `calcom_get_availability` | Checks available time slots | No |
| `calcom_create_webhook` | Creates a webhook subscription | ✏️ Yes |
| `calcom_list_webhooks` | Lists your webhooks | No |
| `calcom_delete_webhook` | Deletes a webhook | ⚠️ Yes |

## 6. Example usage

**"Book a 30-minute intro call with this lead tomorrow at 2pm."**
Behind the scenes: `calcom_list_event_types` → `calcom_get_availability` → `calcom_create_booking`.

**"Move my 3pm meeting with Sam to Thursday morning."**
Behind the scenes: `calcom_list_bookings` → `calcom_get_availability` → `calcom_reschedule_booking`.
