...
|---|---|---|
| `jira_create_issue` | Creates a new issue (bug, task, story, etc.) | ✏️ Yes |
| `jira_get_issue` | Gets a single issue by key | No |
| `jira_update_issue` | Updates fields on an issue | ✏️ Yes |
| `jira_add_comment` | Adds a comment to an issue | ✏️ Yes |
| `jira_list_transitions` | Lists the workflow transitions available for an issue | No |
| `jira_transition_issue` | Moves an issue to a new status | ✏️ Yes |
| `jira_get_issue_history` | Gets the change history of an issue | No |
| `jira_assign_issue` | Assigns an issue to a user | ✏️ Yes |
| `jira_search_users` | Searches for Jira users | No |
| `jira_list_users` | Lists Jira users | No |
| `jira_search_issues` | Searches issues with JQL or filters | No |
| `jira_get_backlog_issues` | Gets issues in a board's backlog | No |
| `jira_get_assigned_issues` | Gets issues assigned to a user | No |
| `jira_list_projects` | Lists Jira projects | No |
| `jira_list_boards` | Lists boards | No |
| `jira_list_sprints` | Lists sprints for a board | No |
| `jira_get_active_sprint` | Gets the currently active sprint for a board | No |
| `jira_get_user_issues_by_email` | Finds issues for a user by their email | No |
| `jira_get_board_user_issues` | Gets a user's issues on a specific board | No |
| `jira_get_sprint_issues` | Lists issues in a sprint | No |
| `jira_get_sprint_status` | Summarizes a sprint's status and progress | No |
| `jira_get_team_workload` | Reports how work is distributed across the team | No |
| `jira_generate_standup_report` | Builds a daily standup report for a board | No |

## 6. Example usage

**"Create a bug in the WEB project titled 'Login button broken on mobile' and assign it to Priya."**
Behind the scenes: `jira_search_users` (to find Priya) → `jira_create_issue` → `jira_assign_issue`.

**"Give me today's standup for the Sprint board and move JIRA-204 to In Progress."**
Behind the scenes: `jira_get_active_sprint` → `jira_generate_standup_report` → `jira_list_transitions` → `jira_transition_issue`.
...
| Transition fails | The target status isn't valid from the current one | Run `jira_list_transitions` first to see allowed transitions |
