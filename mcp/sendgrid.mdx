...
Open **Workflows → New Workflow**. On the canvas, click **+ Add Trigger**. In the **Paths** panel, open the **Apps** tab and select **SendGrid** — its **Triggers** are listed underneath. Use the search box if you have many connectors.

![SendGrid triggers in the Apps panel](/images/mcp/sendgrid/workflows/06-apps-triggers.png)
</Step>
<Step title="Add an action node">
Click the **+** below any node to add a step, then pick **SendGrid** again — this time the panel lists its **Actions**.

![SendGrid actions in the Apps panel](/images/mcp/sendgrid/workflows/07-apps-actions.png)
</Step>
