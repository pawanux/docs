...
Go to [https://tryunleashx.com](https://tryunleashx.com) and open the agent that should transcribe audio.
</Step>
...
![01 transcription tools](/images/mcp/transcription/01-tools.png)

| Tool | What it does | Changes data? |
|---|---|---|
| `transcribe_audio` | Transcribe audio from a URL using the Deepgram or Azure provider | No |
...
→ Runs `transcribe_audio` on the recording URL, then the agent summarizes the returned transcript.

**"Get the text of this Hindi voicemail."**
→ Runs `transcribe_audio` with the language set to Hindi (`hi-IN`) so the transcript comes back accurately.
