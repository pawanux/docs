...
![01 calendly data connector](/images/mcp/calendly/01-data-connector.png)

## 5. Available tools

| Tool (technical) | What it does | Changes data? |
|---|---|---|
| `calendly_list_upcoming_events` | Lists your upcoming scheduled events | No |
| `calendly_get_event_details` | Gets details of a single event | No |
| `calendly_search_events` | Searches your scheduled events | No |
| `calendly_check_availability` | Checks your available time slots | No |
| `calendly_get_busy_times` | Gets the times you're already booked | No |
| `calendly_list_event_types` | Lists your event types | No |
| `calendly_get_event_type_details` | Gets details of a single event type | No |
| `calendly_create_scheduling_link` | Creates a single-use scheduling link | ✏️ Yes |
| `calendly_create_booking` | Creates a booking (schedules an event) | ✏️ Yes |

## 6. Example usage

**"What meetings do I have coming up this week?"**
Behind the scenes: `calendly_list_upcoming_events` (and `calendly_get_event_details` for specifics).

**"Send this prospect a link to book a 30-minute demo."**
Behind the scenes: `calendly_list_event_types` → `calendly_create_scheduling_link`.
