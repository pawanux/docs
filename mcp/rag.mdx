...
Go to [https://tryunleashx.com](https://tryunleashx.com) and open the agent that should search the knowledge base.
</Step>
...
![01 rag tools](/images/mcp/rag/01-tools.png)

| Tool | What it does | Changes data? |
|---|---|---|
| `similarity_search` | Embed a natural-language query and return the top matching knowledge-base entries for this agent | No |
| `fetch_form_details` | Fetch structured form records from the UnleashX datasource by object/field/value | No |
...
→ Runs `similarity_search` over the agent's knowledge base and answers from the closest matching passages.

**"Pull the record where email = jordan@acme.com from the leads form."**
→ Runs `fetch_form_details` against the datasource with that field and value.
