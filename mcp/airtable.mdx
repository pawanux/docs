...
| `schema.bases:write` | Create new tables (only needed for `airtable_create_table`). |
...
|---|---|---|
| `airtable_create_record` | Create a new record in a table. | ✏️ Yes |
| `airtable_get_record` | Read a single record by ID. | No |
| `airtable_update_record` | Update fields on an existing record. | ✏️ Yes |
| `airtable_delete_record` | Permanently delete a record. | ⚠️ Yes |
| `airtable_search_records` | Search records by field, value, formula or date range. | No |
| `airtable_list_bases` | List the bases the token can access. | No |
| `airtable_list_tables` | List tables in a base. | No |
| `airtable_create_table` | Create a new table in a base. | ✏️ Yes |
| `airtable_list_columns` | List the fields/columns of a table. | No |
| `airtable_list_records` | List records in a table. | No |
| `airtable_get_record_ids` | List record IDs in a table. | No |
| `airtable_on_new_record_added` | Detect newly added records since last check (trigger). | No |
| `airtable_clear_snapshots` | Clear stored trigger snapshots so the next check starts fresh. | ⚠️ Yes |

## 6. Example usage

**"Add a row to my 'Leads' table with name Jane Doe, email jane@acme.com, status New."**
Runs `airtable_list_bases` / `airtable_list_tables` to resolve the table, then `airtable_create_record`.

**"Find the lead with email jane@acme.com and mark their status as Contacted."**
Runs `airtable_search_records` to find the record, then `airtable_update_record`.
...
| `404 Not Found` | The base, table or record ID is wrong. | Use `airtable_list_bases` / `airtable_list_tables` to get correct IDs and retry. |
| `422 Unprocessable Entity` | A field name or value doesn't match the table schema. | Check field names with `airtable_list_columns` and retry. |
...
No. Tools call the Airtable API live and return the result. Small snapshots of record IDs are kept only to power the "new record" trigger, and you can clear them with `airtable_clear_snapshots`.
