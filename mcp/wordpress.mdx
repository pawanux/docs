...
Open **Workflows → New Workflow**. On the canvas, click **+ Add Trigger**. In the **Paths** panel, open the **Apps** tab and select **WordPress** — its **Triggers** are listed underneath. Use the search box if you have many connectors.
</Step>
<Step title="Add an action node">
Click the **+** below any node to add a step, then pick **WordPress** again — this time the panel lists its **Actions**.

![WordPress actions in the Apps panel](/images/mcp/wordpress/workflows/07-apps-actions.png)
</Step>
