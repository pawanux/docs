...
|---|---|---|
| `gd_get_file` | Get a file's metadata by ID. | No |
| `gd_find_spreadsheets` | Find spreadsheets (optionally in a folder). | No |
| `gd_find_forms` | Find Google Forms (optionally in a folder). | No |
| `gd_find_folder` | Find a folder by name/parent. | No |
| `gd_find_file` | Find a file by query/folder. | No |
| `gd_find_file_with_changes` | Find files and report changes since last run. | No |
| `gd_list_drive_folders` | List folders in Drive. | No |
| `gd_list_drive_folders_with_changes` | List folders and report changes since last run. | No |
| `gd_on_new_file` | Trigger: return files added since last run. | No |
| `gd_on_new_spreadsheet` | Trigger: return spreadsheets added since last run. | No |
| `gd_copy_file` | Copy a file (optional new name/folder). | ✏️ Yes |
| `gd_create_file_from_text` | Create a file from text content. | ✏️ Yes |
| `gd_delete_file` | Delete a file. | ⚠️ Yes |
| `gd_download_file` | Download a file. | No |
| `gd_move_file` | Move a file to another folder. | ✏️ Yes |
| `gd_share_file` | Share a file with people at a permission level. | ✏️ Yes |
| `gd_update_file` | Update a file's content or name. | ✏️ Yes |
| `gd_search_files` | Search files by term, type, location, date. | No |
| `gd_create_folder` | Create a folder. | ✏️ Yes |
| `gd_delete_folder` | Delete a folder. | ⚠️ Yes |
| `gd_share_folder` | Share a folder with people at a permission level. | ✏️ Yes |
| `gd_create_shared_drive` | Create a shared (team) drive. | ✏️ Yes |
| `gd_delete_shared_drive` | Delete a shared drive. | ⚠️ Yes |
| `gd_get_shared_drive` | Get details of one shared drive. | No |
| `gd_get_many_shared_drives` | List multiple shared drives. | No |
| `gd_update_shared_drive` | Update a shared drive's name/settings/theme. | ✏️ Yes |
| `gd_upload_file` | Upload a file to Drive. | ✏️ Yes |

## 6. Example usage

> **"Find the 'Q3 Reports' folder and share it with finance@acme.com as a commenter."**
>
> Runs `gd_find_folder`, then `gd_share_folder`.

> **"Create a text file called 'meeting-notes' with today's notes in my Projects folder."**
>
> Runs `gd_find_folder` to locate the folder, then `gd_create_file_from_text`.
