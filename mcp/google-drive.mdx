...
Open **Workflows → New Workflow**. On the canvas, click **+ Add Trigger**. In the **Paths** panel, open the **Apps** tab and select **Google Drive** — its **Triggers** are listed underneath. Use the search box if you have many connectors.

![Google Drive triggers in the Apps panel](/images/mcp/google-drive/workflows/06-apps-triggers.png)
</Step>
<Step title="Add an action node">
Click the **+** below any node to add a step, then pick **Google Drive** again — this time the panel lists its **Actions**.

![Google Drive actions in the Apps panel](/images/mcp/google-drive/workflows/07-apps-actions.png)
</Step>
