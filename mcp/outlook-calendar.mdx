...
![01 microsoft authorize](/images/mcp/outlook-calendar/01-microsoft-authorize.png)
...
![02 connected badge](/images/mcp/outlook-calendar/02-connected-badge.png)

## 5. Available tools

| Tool (technical name) | What it does | Changes data? |
|---|---|---|
| `outlook_calendar_search_events` | Searches your calendars for matching events. | No |
| `outlook_calendar_get_event` | Retrieves the details of a single event. | No |
| `outlook_calendar_create_event` | Creates a new calendar event, optionally with attendees. | ✏️ Yes |
| `outlook_calendar_update_event` | Updates an existing event (time, title, attendees, etc.). | ✏️ Yes |
| `outlook_calendar_delete_event` | Deletes / cancels a calendar event. | ⚠️ Yes |
| `outlook_calendar_list_events` | Lists events within a date range. | No |
| `outlook_calendar_list_calendars` | Lists all calendars on the account. | No |
| `outlook_calendar_get_calendar` | Retrieves details of a specific calendar. | No |
| `outlook_calendar_create_calendar` | Creates a new calendar. | ✏️ Yes |
| `outlook_calendar_update_calendar` | Updates a calendar's name or settings. | ✏️ Yes |
| `outlook_calendar_delete_calendar` | Deletes a calendar. | ⚠️ Yes |

## 6. Example usage

**"What's on my calendar tomorrow, and book a 1:1 with Alex at the first free hour?"**
→ Runs `outlook_calendar_list_events` to read tomorrow's schedule, then `outlook_calendar_create_event` to book the 1:1.

**"Move my 3pm design review to Friday and add Maria as an attendee."**
→ Runs `outlook_calendar_search_events` to find the meeting, then `outlook_calendar_update_event` to reschedule and add the attendee.
...
Deleting an event you organize generally sends cancellations, the same as deleting it in Outlook. Treat `outlook_calendar_delete_event` as a real cancellation.
