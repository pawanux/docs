...
Sign in at [https://www.tryunleashx.com](https://www.tryunleashx.com) and open the agent you want to give Firebase access to.
</Step>
...
Open **Workflows → New Workflow**. On the canvas, click **+ Add Trigger**. In the **Paths** panel, open the **Apps** tab and select **Firebase** — its **Triggers** are listed underneath. Use the search box if you have many connectors.
</Step>
<Step title="Add an action node">
Click the **+** below any node to add a step, then pick **Firebase** again — this time the panel lists its **Actions**.

![Firebase actions in the Apps panel](/images/mcp/firebase/workflows/07-apps-actions.png)
</Step>
