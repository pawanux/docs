...
It is a **platform utility**, not a third-party service you sign up for. UnleashX uses it internally to power workflow steps that need to talk to an external API. The single tool it exposes, `http_api_call`, passes through whatever authentication the **target** API requires — you tell it the auth type and value, and it formats the request accordingly.
...
Instead, you supply credentials for the **target** API at call time. `http_api_call` accepts an `auth_type` and `auth_value` and formats the outbound request for you:
...
Sign in at [tryunleashx.com](https://tryunleashx.com) and open the agent or workflow where you want to make HTTP calls.
</Step>
...
The status shows a **Connected** badge. Your agent or workflow can now use the `http_api_call` tool, supplying the target API's URL and auth per call.
</Step>
</Steps>
...
|---|---|---|
| `http_api_call` | Make a generic HTTP request to any URL with custom method, headers, query params, body, and auth. Returns `success`, `status_code`, response `headers`, `body`, and `error`. | ✏️ Depends on the request (read for `GET`; `⚠️`/`✏️` for write methods) |

## 6. Example usage

**"Call our internal inventory API at `https://api.example.com/stock?sku=AB123` and tell me the count."**
→ Runs `http_api_call` with `method=GET` and the SKU as a query parameter, then reads the count from the JSON response.

**"POST this order payload to our fulfillment endpoint using our bearer token."**
→ Runs `http_api_call` with `method=POST`, the JSON `body`, `auth_type=bearer`, and `auth_value` set to your token; returns the endpoint's status and response.
