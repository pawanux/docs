...
Agent deletion in the system is implemented as a **soft delete** mechanism. When an agent is deleted, it is not physically removed from the database. Instead, the agent's status is set to `2` (deleted), and the agent is marked as inactive. This approach preserves data integrity, maintains audit trails, and allows for potential recovery.
...
**Yes**, deleted agents can be restored by updating their status:
