...
The chat agent update system provides multiple endpoints and mechanisms to update existing chat agents. Updates can be performed on complete agents, individual components (status, knowledge base, denied words, dispositions), or specific fields. All update operations maintain data integrity through transactions and change tracking.
...
**Key Difference from Creation:** When `agent_id` is provided in `basic_details`, the system performs an update instead of creation for chat agents.
