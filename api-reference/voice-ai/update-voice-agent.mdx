...
The voice agent update system provides multiple endpoints and mechanisms to update existing voice agents. Updates can be performed on complete agents, individual components (status, knowledge base, denied words, dispositions), or specific fields. All update operations maintain data integrity through transactions and change tracking.
...
**Key Difference from Creation:** When `agent_id` is provided in `basic_details`, the system performs an update instead of creation for voice agents.
...
**Location:** ```2315:2461:src/controllers/agentbuild/agentController.js```
...
2. **Basic Details Update**
   ...
   **Function:** `createOrUpdateAgent(req, transaction)` - Update Branch

   **Location:** ```372:438:src/controllers/agentbuild/agentController.js```

   **Process:**
   - Validates `agent_id` is provided
   - Fetches existing agent record
   - **Agent Behavior Handling:**
     - Retrieves current agent behavior (tone, style, instruction_sensitivity)
   ...
   - Retrieves or generates agent token
   - Generates `AGENT_UID` if missing (for legacy agents)
...
4. **Configuration Update**
   - Updates or creates agent configuration
   - Only updates fields that are provided
   - Tracks configuration changes

5. **Knowledge Base Update**
   - **Tools and Sub-agents:**
     - Compares existing tools/sub-agents with new ones
     - Updates existing entries (enabled/disabled status)
     - Creates new entries for new tools/sub-agents
   ...
   - **Training Status:**
     - Resets agent `TRAINED` status to 0 when knowledge base is updated
...
8. **Agent Sync**
   - Syncs updated agent with AI service asynchronously
...
3. **Tools/Sub-agents Update Logic:**
   - Existing tools/sub-agents are matched by `APP_LOCATION` (ID)
...
1. **Validation:**
   - Validates `agent_id` and `status` are provided
   - Checks agent exists and belongs to workspace

2. **Publish Validation (Status = 1):**
   - For agent type 3 (Telephony):
     - Validates at least one phone number is assigned
     - Throws error if no phone numbers found
   - For other agent types:
     - Validates agent type is set
...
4. **Special Handling for Deletion (Status = 2):**
   - Calls Millis AI API to delete agent from external platform
   - Deletes knowledge base from AI service
   - Extracts `platform_agent_id` from `AGENT_METADATA`
...
6. **Response:**
   ...
   ```json
   {
     "message": "Agent Published Successfully", // or "Unpublished" or "Deleted"
     "data": {
       "id": 123,
       "name": "Agent Name",
       "status": 1
...
### 3. Update Knowledge Base Status (Voice Agents)

**Endpoint:** `POST /api/agent/updateKnowledgeBaseStatus`

**Authentication:** Requires Super Admin authentication

**Controller:** `src/controllers/agentbuild/agentController.js` - `updateKnowledgeBaseStatus`

**Location:** ```3464:3549:src/controllers/agentbuild/agentController.js```
...
2. **Agent Validation:**
   - Verifies associated agent exists and is not deleted

3. **Status Update:**
   - Updates knowledge base status
   - Sets `TRAINED` to 0 (not trained)

4. **Agent Training Reset:**
   - Resets agent's `TRAINED` status to 0
   - Indicates agent needs retraining after knowledge base change
...
### 4. Update Denied Word and Disposition Status (Voice Agents)

**Endpoint:** `POST /api/agent/updateDeniedWordAndDisposition`

**Authentication:** Requires Super Admin authentication

**Controller:** `src/controllers/agentbuild/agentController.js` - `updateDeniedWordAndDisposition`

**Location:** ```3557:3677:src/controllers/agentbuild/agentController.js```
...
When updating agent behavior fields (tone, style, instruction_sensitivity):

**Location:** ```380:394:src/controllers/agentbuild/agentController.js```
...
### 4. Tools and Sub-agents Update Logic

**Location:** ```955:1014:src/controllers/agentbuild/agentController.js```

**Update Strategy:**
1. **Existing Tools/Sub-agents:**
   - Matched by `APP_LOCATION` (ID)
   - Updated with new `IS_ENABLED` status
   - Status set to 2 (deleted) if disabled, 1 (active) if enabled

2. **New Tools/Sub-agents:**
   - Created as new entries
   - Assigned to agent

3. **Removed Tools/Sub-agents:**
...
**Location:** ```881:886:src/controllers/agentbuild/agentController.js```

- Agent's `TRAINED` status is reset to 0
- Indicates agent needs retraining
- Knowledge base sync is triggered

### 6. Token Management

**Location:** ```112:169:src/controllers/agentbuild/agentController.js```

- Agent token is retrieved or regenerated during updates
- Token is used for agent authentication
- Token includes agent ID, user ID, company ID, workspace ID
...
2. **Agent Configuration Sync:**
   - Syncs agent configuration with AI service
   - Updates AI service with new agent settings

3. **External API Calls (Type 3 Agents):**
   - For telephony agents, calls external agent API
   - Updates agent metadata in external platform

## Update Validation Rules

### 1. Agent Existence
- Agent must exist in database
- Agent must belong to workspace
- Agent must not be deleted (STATUS != 2)

### 2. Publish Validation
- Agent type 3 (Telephony) must have at least one phone number
- Agent type must be set for all agent types
...
- Invalid agent IDs throw "Agent not found" errors
- Invalid status values throw validation errors

### 3. Business Logic Errors
- Publishing agent without phone numbers throws error
...
module.exports.createOrUpdateCompleteAgent = async (req, res, next) => {
  // Main agent update handler
}
```

### Basic Details Update

```372:438:src/controllers/agentbuild/agentController.js
if (postData.agent_id) {
  // Update agent build settings
...
```3464:3549:src/controllers/agentbuild/agentController.js
...
```3557:3677:src/controllers/agentbuild/agentController.js
...
1. **Agent ID Required:** Updates require `agent_id` in `basic_details`
...
4. **Tools/Sub-agents:** Updates existing entries instead of only creating
5. **Token Refresh:** Updates refresh agent token instead of generating new one
...
8. **AGENT_UID:** Updates only generate AGENT_UID if missing (for legacy agents)
...
4. **Validation:** Always validate agent exists before updating
...
- Super Admin required for complete agent updates and knowledge base updates
