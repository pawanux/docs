...
Voice agent deletion in the system is implemented as a **soft delete** mechanism. When a voice agent is deleted, it is not physically removed from the database. Instead, the voice agent's status is set to `2` (deleted), and the voice agent is marked as inactive. This approach preserves data integrity, maintains audit trails, and allows for potential recovery.
